...
_ویندوز:_ فایل `C:\Windows\System32\drivers\etc\hosts` را با Notepad (به عنوان Administrator) باز کنید و این خط را اضافه کنید:

```
192.168.1.100    survey.company.local
```

_مک/لینوکس:_ فایل `/etc/hosts` را ویرایش کنید:
...
- یک فایل CSV یا TXT با این فرمت آماده کنید (رمز عبور می‌تواند هر طول و ترکیبی باشد — هیچ محدودیتی روی حروف بزرگ/کوچک، عدد یا نماد وجود ندارد):

```
username,نام کامل,رمز عبور,نقش
ali_mohammadi,علی محمدی,Pass1234,employee
sara_ahmadi,سارا احمدی,sara2026,employee
manager1,مدیر واحد,Admin9012,admin
```
...
- عنوان و سوالات را وارد کنید — برای هر سوال می‌توانید امتیاز عددی (۱ تا ۱۰)، امتیاز ایموجی (بد/متوسط/خوب/عالی)، توضیح متنی، یا هر ترکیبی از این سه را فعال کنید و هرکدام را اختیاری یا الزامی کنید
