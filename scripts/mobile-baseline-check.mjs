...
async function prepareRoutes() {
  ...
  const routes = [
    ...
    {
      id: 'admin-ip-audit-filtered',
      role: 'admin',
      path: surveyId ? `/admin/surveys/${surveyId}/results` : null,
      state: 'filtered',
      setup: 'open-ip-audit-filtered',
      mobileOnly: true,
    },
...
async function openAndVerifyIPAudit(page, route, viewport, theme) {
  const tab = page.getByRole('tab', { name: 'ممیزی پاسخ‌های IP', exact: true });
  const tabVisible = await tab.isVisible().catch(() => false);
  if (!tabVisible) {
    report.findings.push({
      severity: 'high', route: route.id, viewport: viewport.key, theme,
      rule: 'ip-audit-tab', detail: 'The IP audit results tab was not visible.',
    });
    return { tabVisible, sectionVisible: false };
  }

  await tab.click();
  const section = page.getByTestId('ip-response-audit');
  await section.waitFor({ state: 'visible', timeoutMs: 5_000 }).catch(() => {});
  const sectionVisible = await section.isVisible().catch(() => false);
  let filterSummaryVisible = null;
  let clearFiltersVisible = null;

  if (route.setup === 'open-ip-audit-filtered' && sectionVisible) {
    await page.getByPlaceholder('جستجو بر اساس آدرس IP...', { exact: true }).fill('172.19');
    await page.waitForTimeout(450);
    const summary = page.getByTestId('ip-audit-filter-summary');
    filterSummaryVisible = await summary.isVisible().catch(() => false);
    clearFiltersVisible = filterSummaryVisible
      ? await summary.getByRole('button', { name: 'پاک کردن فیلترها', exact: true }).isVisible().catch(() => false)
      : false;
  }

  const evidence = { tabVisible, sectionVisible, filterSummaryVisible, clearFiltersVisible };
  if (!sectionVisible || filterSummaryVisible === false || clearFiltersVisible === false) {
    report.findings.push({
      severity: 'high', route: route.id, viewport: viewport.key, theme,
      rule: 'responsive-ip-audit', detail: JSON.stringify(evidence),
    });
  }
  return evidence;
}
...
async function captureRoute(page, route, viewport, theme) {
  ...
  let ipAuditEvidence = null;
  if (['open-ip-audit', 'open-ip-audit-filtered'].includes(route.setup)) {
    ipAuditEvidence = await openAndVerifyIPAudit(page, route, viewport, theme);
  }
