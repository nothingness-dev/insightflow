...
<div
          data-testid="survey-detail-actions"
          className="grid grid-cols-2 gap-2 mt-5 pt-4 border-t border-gray-100 sm:flex sm:flex-wrap sm:items-center"
        >
          {survey.status === 'draft' && (
            <>
              <Link
                to={`/admin/surveys/${id}/edit`}
                className="btn-secondary order-2 w-full justify-center whitespace-nowrap text-sm sm:order-none sm:w-auto"
              >
                ویرایش
              </Link>
              <button
                type="button"
                data-testid="survey-detail-primary-action"
                onClick={() => setPublishModal(true)}
                className="btn-success order-1 col-span-2 w-full justify-center whitespace-nowrap text-sm sm:order-none sm:w-auto"
              >
                انتشار نظرسنجی
              </button>
            </>
          ...
          {survey.status === 'published' && (
            <button
              ...
              className="btn-danger order-2 w-full justify-center whitespace-nowrap text-sm sm:order-none sm:w-auto"
            >
              بستن نظرسنجی
            </button>
          )}
          {(survey.status === 'closed' || survey.status === 'published') && (
            <Link
              ...
              data-testid="survey-detail-primary-action"
              className="btn-primary order-1 col-span-2 flex w-full items-center justify-center gap-2 whitespace-nowrap text-sm sm:order-none sm:w-auto"
            >
          ...
          <button
            ...
            className={`btn-danger order-3 flex w-full items-center justify-center gap-1.5 whitespace-nowrap text-sm sm:order-none sm:ms-auto sm:w-auto ${survey.status === 'closed' ? 'col-span-2' : ''}`}
          >
            <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
