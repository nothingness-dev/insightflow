...
const ProgressIcon = () => <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.8}><path strokeLinecap="round" strokeLinejoin="round" d="M3 3v18h18M7 16l4-5 3 3 5-7" /></svg>;
const SurveyIcon = () => <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.8}><path strokeLinecap="round" strokeLinejoin="round" d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2m-6 9l2 2 4-4" /></svg>;
const UsersIcon  = () => <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.8}><path strokeLinecap="round" strokeLinejoin="round" d="M12 4.354a4 4 0 110 5.292M15 21H3v-1a6 6 0 0112 0v1zm0 0h6v-1a6 6 0 00-9-5.197M13 7a4 4 0 11-8 0 4 4 0 018 0z" /></svg>;
...
export default function AdminLayout({ children }: { children: ReactNode }) {
  ...
  const Sidebar = ({ mobile = false }) => (
    <aside className={`flex flex-col h-full bg-white border-l border-gray-100 shadow-sm ${mobile ? 'w-full' : 'w-64'}`}>
<div className="flex items-center justify-between px-6 py-5 border-b border-gray-100">
        ...
        {mobile && (
          <button onClick={() => setSidebarOpen(false)} className="text-gray-400 hover:text-gray-600"><CloseIcon /></button>
        )}
      </div>
...
<div className="px-4 py-4 border-t border-gray-100">
  ...
  return (
    <div className="flex h-[100dvh] overflow-hidden" style={{ backgroundColor: 'var(--c-bg)' }} dir="rtl">
...
<header className="bg-white border-b border-gray-100 px-4 sm:px-6 py-3 sm:py-4 flex items-center justify-between gap-3">
          <div className="flex items-center gap-4">
            <button aria-label="باز کردن منوی مدیریت" className="lg:hidden touch-target text-gray-500 hover:text-gray-700" onClick={() => setSidebarOpen(true)}>
              <MenuIcon />
            </button>
            <p className="text-sm text-gray-400 hidden sm:block">سامانه نظرسنجی سازمانی</p>
          </div>
          <ThemeSwitcher />
        </header>
